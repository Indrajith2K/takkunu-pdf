...
import { Upload, FileIcon, X, ArrowRight, Image as ImageIcon } from 'lucide-react';

const PdfToJpg = ({ onNavigate }) => {
    ...
    const handleConvert = async () => {
        ...
        try {
            const blob = await pdfApi.pdfToJpg(file);
            ...
            link.href = url;
            link.setAttribute('download', 'takkunu-images.zip');
            ...
            console.error(err);
            setError('Conversion failed. Ensure the PDF is not password protected.');
        } finally {
                ...
                <div className="max-w-3xl mx-auto">
                    {isProcessing ? (
                        <ProcessingCard
                            ...
                            progress={progress}
                            steps={[
                                { threshold: 20, label: "Scanning PDF pages" },
                                { threshold: 60, label: "Rendering High-Res JPGs" },
                                { threshold: 100, label: "Zipping images..." }
                            ]}
                        />
                    ) : (
                        <>
                            <div className="text-center mb-12">
                                <h1 className="serif-heading text-4xl md:text-5xl mb-4">PDF to JPG</h1>
                                <p className="text-slate-400">Convert each PDF page into a high-quality image.</p>
                            </div>
                            ...
                            <div className="glass-card rounded-3xl p-8 border border-white/10 relative overflow-hidden">
                                {!file ? (
                                    <div className="border-2 border-dashed border-white/10 rounded-2xl p-12 text-center bg-white/5 hover:bg-white/10 transition-colors relative">
                                        ...
                                        />
                                        <ImageIcon className="w-12 h-12 text-brand-purple mx-auto mb-4" />
                                        <p className="text-xl font-medium">Upload PDF</p>
                                        <p className="text-sm text-slate-500 mt-2">Extracts all pages as images</p>
                                    </div>
                                ) : (
